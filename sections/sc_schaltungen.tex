...
\begin{multicols}{2}
	\paragraph{Grundschaltung}~\\
		\adjustbox{scale=1}{\input{tikz/sc_grundschaltung}} 
	...
	\paragraph{Timing}~\\
	\begin{center}
		\adjustbox{scale=1.2}{\input{tikz/sc_timing}} \\
...
\begin{tabularx}{\linewidth}{p{0.25\linewidth}XX}
	...
	Kapazität & \input{tikz/block_c} & $Y_{c}(z) = C \cdot (1-z^{-1})$ \\
	Geschaltetes C & \input{tikz/block_sc1} & $Y_{sc}(z) = C$ \\
	Geschaltetes C mit Inversion & \input{tikz/block_sc2} & $Y_{sc}^{eo}(z) = -C \cdot z^{-1/2}$ \newline $Y_{sc}^{ee}(z) = -C \cdot z^{-1}$ \\
	\hline
\end{tabularx}
...
\begin{tabularx}{\linewidth}{p{0.25\linewidth}XX}
	...
	Opamp als Tiefpass & \input{tikz/block_opt_sc}& $\Zop(z) = - \frac{\frac{1}{C_f + C_r}}{1-\frac{C_f}{C_f+C_r}z^{-1}}$ \\
	\hline
